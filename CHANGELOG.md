...
**Richer right-click menus, and image-handling fixes** (#42)
A pass over context menus and canvas image behaviour, so the same actions work everywhere and stray gestures stop biting:

- **Slide thumbnails** gain **Cut / Copy / Paste Slide(s)** (also `Ctrl+C/X/V` when the navigator has focus and no element is selected) — multi-selected thumbnails move as a set, and Paste lands right after the slide you clicked — plus **Import Slides…**, which merges another `.dek` bundle's slides in at that point, copying their images into the current deck's asset store (`importSlidesFromWorkspaceDeck`, whatever backend the current deck uses; the source deck is never touched).
- **Canvas images** (freeform boxes) get **Copy / Paste / Download Image** and **Add Link (from Clipboard)** — an `http(s)`/`mailto` URL on the clipboard makes the box clickable, the paste-driven twin of dragging a link on — alongside the existing Fit / Replace / Remove.
- **Layout images** (Text + Image, Image – Full, Image + Caption) now open that *same* Dek menu on right-click instead of the browser's native one (Copy / Paste / Download, Fit: Cover / Contain, Replace…, Remove), wired to the slide's `image` field. Add Link stays freeform-only — semantic images have no per-image link.
- **Fill / fit toggle** for those layouts: they always `cover`-cropped, so a tall image — a logo with a wordmark under it — lost its edge with no way out. *fit* (`contain`) now shows the whole image letterboxed; stored as `imageFit` and carried across layout switches (Image + Caption defaults to *fit*, the others to *fill*).
- **Scroll-zoom floors at the natural fit** (scale 1). Zooming out below it only shrank the picture inside its frame, reading as a crop; you now zoom *in* to reframe and use the fill/fit toggle to see the whole image. Baked PowerPoint crops are unaffected (they live in the pixels, not a sub-1 zoom).
- **Drag-and-drop fixes.** The drop highlight no longer flickers as the pointer crosses onto child boxes (`dragleave` now checks `relatedTarget`). And a drop that lands a hair outside a box — or on an empty slide whose canvas is `pointer-events: none` — no longer hits the browser default and **opens the image in a new tab, losing the deck**: a window-level guard swallows stray file/URL drops outside real text fields, while the app's own drop handlers still fire.
